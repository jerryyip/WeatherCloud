...
const char *pixels_patterns[] = {"sleep", "wakeup", "wait", "answer", "offline", "online","lightning","sunny","cloudy"};
...
void sunny() {
    for (int i = 0; i < PIXELS_NUM; i++) {
        pixels.setPixelColor(i, 240, 180, 34);
    }
    pixels.show();
    delay(3000);
    pixels.clear();
...
void cloudy() {
    for (int i = 0; i < PIXELS_NUM; i++) {
        pixels.setPixelColor(i, 152, 245, 255);
    }
    pixels.show();
    delay(3000);
    pixels.clear();
